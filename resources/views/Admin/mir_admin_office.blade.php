        ...
        <tr>
          ...
          <td>
            <a href="{{ route('admin_office_page',['id'=>$value->id()]) }}"
               class="btn btn-primary btn-xs mr-border-radius-5 fa fa-eye"></a>
            {!! MrBtn::loadForm('office_edit', 'Admin\\MrAdminOfficeEditForm', ['id' =>$value->id()], '', ['btn btn-primary btn-xs fa fa-edit']) !!}
            <a href="/admin/office/delete/{{ $value->id() }}" onclick="return confirm('Вы уверены?');" class="btn btn-danger btn-xs mr-border-radius-5 fa fa-trash-alt"></a></td>
        </tr>
      @endforeach
      </tbody>
    </table>
